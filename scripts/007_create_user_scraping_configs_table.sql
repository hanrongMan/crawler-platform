...
CREATE TABLE IF NOT EXISTS user_scraping_configs (
    ...
    website_type TEXT NOT NULL DEFAULT 'custom',
...
-- 创建唯一约束，确保每个用户只有一个默认配置
CREATE UNIQUE INDEX IF NOT EXISTS user_scraping_configs_user_default_idx 
ON user_scraping_configs (user_id, is_default) 
WHERE is_default = true;

-- 创建索引以提高查询性能
CREATE INDEX IF NOT EXISTS user_scraping_configs_user_id_idx 
ON user_scraping_configs (user_id);
